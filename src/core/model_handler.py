...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        if config['data_type'] in ('text'):
            ...
            self.model = Model(config, train_set=datasets.get('train', None))  # 会调用model 的
            self.model.network = self.model.network.to(self.device)


            self._n_train_examples = 0
            if train_set:
                self.train_loader = DataStream(train_set, self.model.vocab_model.word_vocab, config=config, isShuffle=True, isLoop=True, isSort=True)  # 变成一个batch 的流
    ...
    def train(self):
        ...
        while self._stop_condition(self._epoch, self.config['patience']):
            ...
            # Validation phase
            dev_output, dev_gold = self.run_epoch(self.dev_loader, training=False, test=False, verbose=self.config['verbose'],
            ...
            if self._best_metrics[self.config['eary_stop_metric']] < cur_dev_score:  # 更新模型
    ...
    def test(self):
        ...
        output, gold = self.run_epoch(self.test_loader, training=False, test=True, verbose=0,
                                 out_predictions=self.config['out_predictions'])
        ...
        if self.config['out_predictions']:
            test_score = self.model.score_func(gold, output, True)
    ...
    def batch_IGL_stop(self, x_batch, step, training, test, out_predictions=False):
        ...
        context, context_lens, targets, sen_adj, dep_adj = x_batch['context'], x_batch['context_lens'], x_batch['targets'], x_batch['sen_adj'], x_batch['dep_adj']

        context2 = x_batch.get('context2', None)
        context2_lens = x_batch.get('context2_lens', None)

        raw_context_vec, context_vec, context_mask = network.prepare_init_graph(context, context_lens)
        ...
        cur_raw_adj, cur_dep_adj = network.learn_graph(network.graph_learner, raw_node_vec, graph_skip_conn = network.graph_skip_conn1, node_mask=node_mask,
                                                   graph_include_self=network.graph_include_self, init_adj=dep_adj)  # 此处为先加依赖
        node_vec = torch.relu(network.encoder.graph_encoders[0](init_node_vec, cur_dep_adj))
        node_vec = F.dropout(node_vec, network.dropout, training=network.training)


        cur_raw_adj, cur_sen_adj = network.learn_graph(network.graph_learner, raw_node_vec,
                                                   graph_skip_conn=network.graph_skip_conn, node_mask=node_mask,
                                                   graph_include_self=network.graph_include_self, init_adj=sen_adj)

        # Add mid GNN layers
        t = 1
        for encoder in network.encoder.graph_encoders[1:-1]:
            if t % 2 == 1:
                node_vec = torch.relu(encoder(node_vec, cur_sen_adj))
                node_vec = F.dropout(node_vec, network.dropout, training=network.training)

            else:
                node_vec = torch.relu(encoder(node_vec, cur_dep_adj))
                node_vec = F.dropout(node_vec, network.dropout, training=network.training)
        ...
        # BP to update weights
        if t%2 == 1:
            output = network.encoder.graph_encoders[-1](node_vec, cur_sen_adj)
        else:
            output = network.encoder.graph_encoders[-1](node_vec, cur_dep_adj)
        output = network.compute_output(output, context_vec=init_node_vec, test=is_test, node_mask=node_mask)
        ...
        if self.config['graph_learn'] and self.config['graph_learn_regularization']:
            loss1 += self.add_batch_graph_loss(cur_raw_adj, raw_node_vec)  # loss1是图正则化的Loss

        first_raw_adj, first_dep_adj,  first_sen_adj\
            = cur_raw_adj, cur_dep_adj,cur_sen_adj
        ...
        batch_all_outputs = []
        while self.config['graph_learn'] and (iter_ == 0 or torch.sum(batch_stop_indicators).item() > 0) and iter_ < max_iter_:  # 迭代学习，，torch.sum(batch_stop_indicators).item()  torch中所有元素的和
            iter_ += 1
            batch_last_iters += batch_stop_indicators

            pre_raw_adj = cur_raw_adj
            cur_raw_adj, cur_dep_adj = network.learn_graph(network.graph_learner2, node_vec, graph_skip_conn = network.graph_skip_conn1,
                                                       node_mask=node_mask, graph_include_self=network.graph_include_self, init_adj=dep_adj) # gai
            cur_raw_adj, cur_sen_adj = network.learn_graph(network.graph_learner2, node_vec,
                                                           graph_skip_conn=network.graph_skip_conn, node_mask=node_mask,
            ...
            # BP to update weights
            if (t % 2 == 1):
                tmp_output = network.encoder.graph_encoders[-1](node_vec, cur_sen_adj)
            else:
                tmp_output = network.encoder.graph_encoders[-1](node_vec, cur_dep_adj)
            tmp_output = network.compute_output(tmp_output, context_vec=init_node_vec,test=is_test, node_mask=node_mask)
            ...
            tmp_stop_criteria = batch_diff(cur_raw_adj, pre_raw_adj, first_raw_adj) > eps_adj  # 停止迭代的条件 差还大时，tmp_stop_criteria 为1，否则是0.batch_stop_indicators不满足迭代条件，退出迭代。eps_adj 为可人为调的阈值
        ...
        if iter_ > 0:
            ...
            loss = torch.mean(loss / batch_last_iters.float()) + loss1   # loss1是图正则化的Loss
            ...
            score = self.model.score_func(targets.cpu(), output.detach().cpu())
            score1 = self.model.score_func1(targets.cpu(), output.detach().cpu()) #jia


        else:
            loss = loss1
            score = self.model.score_func(targets.cpu(), output.detach().cpu())
            score1 = self.model.score_func1(targets.cpu(), output.detach().cpu())


        res = {'loss': loss.item(),
                'metrics': {'nloss': -loss.item(), self.model.metric_name: score, self.model.metric_name1: score1}
        }
        if out_predictions:
            res['predictions'] = output.detach().cpu()
        ...
        if training:
            loss = loss / self.config['grad_accumulated_steps'] # Normalize our loss (if averaged)
            loss.backward(retain_graph=True)  # jia
    ...
    def _run_whole_epoch(self, data_loader, training=True, verbose=None, out_predictions=False):
        ...
        while self.config['graph_learn'] and (iter_ == 0 or diff(cur_raw_adj, pre_raw_adj, first_raw_adj).item() > eps_adj) and iter_ < max_iter_:
            ...
            if update_adj_ratio is not None:
                cur_adj = update_adj_ratio * cur_adj + (1 - update_adj_ratio) * first_adj  # 后面g
    ...
    def _run_batch_epoch(self, data_loader, training, test,rl_ratio=0, verbose=10, out_predictions=False):
        ...
        for step in range(data_loader.get_num_batch()):
            ...
            if self.config.get('no_gnn', False):
                ...
                res = self.batch_IGL_stop(x_batch, step, training=training, test=test,  out_predictions=out_predictions)
    ...
    def add_graph_loss(self, out_adj, features):
        ...
        graph_loss += (-self.config['smoothness_ratio']) * torch.trace(torch.mm(features.transpose(-1, -2), torch.mm(L, features))) / int(np.prod(out_adj.shape))  #加负号
