...
def multi_run_main(config):
    ...
    for cnf in configs:
        ...
        acc.append(test_metrics[model.model.metric_name])
        f1.append(test_metrics[model.model.metric_name1])

    print('Max acc: {}'.format(np.max(acc)))
    print('Max f1: {}'.format(np.max(f1)))
...
def get_args():
    ...
    parser.add_argument('--multi_run', action='store_true', help='flag: multi run')  # store_true 默认是false
